...
import { CheckCircle2, AlertCircle, Search, Loader2, Clock } from "lucide-react"

type EmployeResult = { id: string; prenom: string; nom: string; poste: string; matricule: string }
type Done = { prenom: string; nom: string; dateEntree: string }

export default function PointerEntreePage() {
  ...
  async function pointer() {
    ...
    const res = await fetch("/api/pointage/entree", {
      ...
      body: JSON.stringify({ employeId: selected.id }),
    })
    const d = await res.json()
    if (res.ok) {
      setDone({ prenom: selected.prenom, nom: selected.nom, dateEntree: d.dateEntree })
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4"
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 text-center max-w-sm w-full">
          ...
          <h2 className="text-xl font-black text-slate-900">Arrivée enregistrée !</h2>
          <p className="text-slate-600 font-semibold mt-1">{done.prenom} {done.nom}</p>
          <p className="text-3xl font-mono font-black text-emerald-600 mt-3">
            {new Date(done.dateEntree).toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" })}
          </p>
          <p className="text-xs text-slate-400 mt-1">
            {new Date(done.dateEntree).toLocaleDateString("fr-FR", { weekday: "long", day: "numeric", month: "long" })}
          </p>
          <button onClick={() => setDone(null)}
            className="mt-6 w-full py-3 rounded-2xl bg-emerald-600 text-white font-bold text-sm hover:bg-emerald-700 transition-colors">
            Nouveau pointage
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen flex items-center justify-center p-4"
      ...
      <div className="bg-white rounded-3xl shadow-2xl p-6 max-w-sm w-full">
        <div className="text-center mb-6">
          ...
          <h1 className="text-lg font-black text-slate-900">Pointage — Entrée</h1>
          <p className="text-xs text-slate-400 mt-1">
        ...
        <div className="relative">
          <div className="flex items-center gap-2 border-2 border-slate-200 rounded-xl px-3 py-2.5 focus-within:border-emerald-400 transition-colors">
            {searching
            ...
            <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl z-10 overflow-hidden">
              {results.map(e => (
                <button key={e.id}
                  ...
                  <div className="h-8 w-8 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-700 text-xs font-black flex-shrink-0">
                    {e.prenom[0]}{e.nom[0]}
                  </div>
        ...
        {selected && (
          <div className="mt-3 flex items-center gap-3 p-3 rounded-xl bg-emerald-50 border border-emerald-200">
            <div className="h-9 w-9 rounded-full bg-emerald-200 flex items-center justify-center text-emerald-800 text-sm font-black flex-shrink-0">
              {selected.prenom[0]}{selected.nom[0]}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-sm font-bold text-slate-900 truncate">{selected.prenom} {selected.nom}</p>
              <p className="text-xs text-emerald-600">{selected.poste} · {selected.matricule}</p>
            </div>
          </div>
        )}
        ...
        <button onClick={pointer} disabled={!selected || saving}
          className="mt-4 w-full py-3.5 rounded-2xl bg-emerald-600 text-white font-bold text-sm hover:bg-emerald-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2">
